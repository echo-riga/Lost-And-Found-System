...
SELECT m.*, 
       ...
       u1.name AS owner_username,    -- Username of the item owner
       u2.name AS proof_username    -- Username of the proof item owner (initiator)
FROM matches m
JOIN items owner ON m.owner_item_id = owner.item_id
JOIN items proof ON m.proof_item_id = proof.item_id
JOIN users u1 ON owner.user_id = u1.user_id  -- Fetch owner username
JOIN users u2 ON proof.user_id = u2.user_id  -- Fetch proof item owner's username
WHERE owner.user_id = ? AND m.final_status = 'unresolve';
";
